  ...
  async searchDrug(query: string): Promise<NormalizedDrugData | null> {
    try {
      ...
      return {
        ...
        indications: result.indications_and_usage?.[0] || result.purpose?.[0] || result.description?.[0],
        dosage: result.dosage_and_administration?.[0] || result.how_to_use?.[0] || result.instructions_for_use?.[0],
        warnings: result.warnings?.[0] || result.boxed_warning?.[0] || result.precautions?.[0] || result.do_not_use?.[0],
        side_effects: result.adverse_reactions?.[0] || result.side_effects?.[0]
      };
    } catch (error: any) {
